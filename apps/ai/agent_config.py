...
_MEETING_SUMMARY = (
    ...
    "ONLY a JSON object {\"summary\": str, \"action_items\": [str]}.\n"
    "summary: 1-3 tight sentences of what ACTUALLY happened — the specific wins, "
    "blockers, decisions and people NAMED in the notes, in the notes' own terms "
    "VERBATIM where possible. Keep concrete nouns intact ('the recognition feed "
    "slice', 'the design review for the check-in form') — never flatten them to "
    ...
    "worth noting').\n"
    "action_items: 0-6 next steps, each stating WHO does WHAT (and to whom / by when "
    "if the notes say so). Take the OWNER from the notes — the person the notes say is "
    "responsible; if the notes name no owner, phrase the step WITHOUT inventing a "
    "name. e.g. notes 'Lin will sort the export' -> 'Lin to fix the flaky data "
    "export'; notes with no named owner -> 'Follow up on the design review for the "
    "check-in form with the design team'. Each must move work FORWARD; merely "
    "restating a blocker ('unblock the design review') is NOT an action item. If the "
    "notes imply no action, return [].\n"
