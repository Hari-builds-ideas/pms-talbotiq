...
def finalize(review, actor):
    """APPROVED -> FINALIZED. HITL gate, layer 1.

    Demands state == APPROVED AND human_reviewer set; anything else is 422
    HITL_APPROVAL_REQUIRED (not 409 — the failure is "no human approval", which
    is the message the caller needs). Single-step for now; Module 5's approval
    matrix will replace this body with multi-step routing (see module docstring).
    """
