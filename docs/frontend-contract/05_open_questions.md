...
|---|----------|---------------------------------------------|-----------------|
| 1 | **Pagination** — only the audit console paginates (`{count,next,previous,results}`, 50/page, max 200). Other list endpoints (goals, reviews, JDs, positions, roadmaps, integrations, succession) return **plain arrays**. | MVP renders non-audit lists as full arrays (volumes are tenant-scoped + modest). | If any list can grow large, add DRF pagination server-side first; the frontend should code list views to tolerate either shape. **Backend follow-up.** |
